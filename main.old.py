...
def from_base_10_to_n(base_n: int, remaining: int, numbers: list = [], fractions: list = [], is_negative = False, last_exponent: int or None = None) -> list:
    ...
    print((factor, remaining, exponent))

    if (factor >= base_n):
        warnings.warn(f"Found a number which is greater than the base. Input number might not be in base {base_n}.")
    ...
    if (exponent >= 0):
        ...
        while len(fractions) < abs(exponent)-1:
                fractions += [0]
    ...
    if(remaining > 0):
        ...
        if exponent > 0:
            numbers += [0]*(exponent-len(numbers)+1)
